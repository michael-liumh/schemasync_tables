...
def sync_schema(fromdb, todb, options, filter_tables=None):
    ...
    Args:
        ...
        filter_tables: List (default=None), filter tables?
    ...
    for t in fromdb.tables:
        if filter_tables and t not in filter_tables:
            continue
...
def sync_created_tables(from_tables, to_tables,
                        sync_auto_inc=False, sync_comments=False, filter_tables=None):
...
def sync_views(fromdb, todb, filter_views=None):
    src = fromdb.views
    dest = todb.views

    if filter_views:
        for k in src:
            if k in filter_views:
                del src[k]
...
def sync_procedures(fromdb, todb, filter_procedures=None):
    src = fromdb.procedures
    dest = todb.procedures

    if filter_procedures:
        for k in src:
            if k in filter_procedures:
                del src[k]
...
def sync_triggers(fromdb, todb, filter_triggers=None):
    src = fromdb.triggers
    dest = todb.triggers

    if filter_triggers:
        for k in src:
            if k in filter_triggers:
                del src[k]
